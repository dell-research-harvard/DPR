...
def init_hf_bart_biencoder(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_bart_biencoder_components

    return get_bart_biencoder_components(args, **kwargs)
...
def init_hf_bart_tenzorizer(args, **kwargs):
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_bart_tensorizer

    return get_bart_tensorizer(args)
...
BIENCODER_INITIALIZERS = {
    ...
    "hf_bart": init_hf_bart_biencoder,
...
TENSORIZER_INITIALIZERS = {
    ...
    "hf_bart": init_hf_bart_tenzorizer,
