...
from transformers.modeling_bart import BartConfig, BartModel
...
def get_bart_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    question_encoder = HFBartEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    ctx_encoder = HFBartEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    tensorizer = get_bart_tensorizer(cfg)
    return tensorizer, biencoder, optimizer
...
def get_bart_tensorizer(cfg, tokenizer=None):
    sequence_length = cfg.encoder.sequence_length
    pretrained_model_cfg = cfg.encoder.pretrained_model_cfg

    if not tokenizer:
        tokenizer = get_bart_tokenizer(pretrained_model_cfg, do_lower_case=cfg.do_lower_case)
        if cfg.special_tokens:
            _add_special_tokens(tokenizer, cfg.special_tokens)

    return BartTensorizer(tokenizer, sequence_length)
...
def get_bart_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True):
    return BartTokenizer.from_pretrained(pretrained_cfg_name, do_lower_case=do_lower_case)
...
class HFBartEncoder(BartModel):
    ...
    def init_encoder(
        ...
        cfg = BartConfig.from_pretrained(cfg_name if cfg_name else "bert-base-uncased")
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        ...
        if pretrained:
            ...
            return HFBartEncoder(cfg, project_dim=projection_dim)
...
class BartTensorizer(Tensorizer):
    def __init__(self, tokenizer: BartTokenizer, max_length: int, pad_to_max: bool = True):
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.pad_to_max = pad_to_max
